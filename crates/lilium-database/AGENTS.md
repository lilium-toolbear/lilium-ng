...
- `lilium-database` owns database runtime, transactions, connection owners, and SeaORM entity definitions for the current architecture.
- Service crates should use this crate for entity modules, connection ownership, transactions, and dedicated PostgreSQL connection types.
...
- Add a SeaORM entity for a real table before writing service-layer CRUD against that table.
- Register every new entity in `entities/mod.rs` and its `prelude` module.
